...
function pingForRTT () {
    serverAdress = document.getElementById('addressInput').value;
    if (!serverAdress) {
    ...
    client.on('message', function (incommingMessage, remote) {
        ...
        document.getElementById('RTTValue').innerHTML = roundTripTime + 'ms';
    });
}


// Send 10 requests og udregn gennemsnits svartiden
async function pingForAverageRTT (amount = 10, delay = 500) {
    serverAdress = document.getElementById('addressInput').value;
    if (!serverAdress) {
        alert("Please enter a server address")
    }
    ...
    client.on('message', function (incommingMessage, remote) {
        ...
        document.getElementById('averageRTTValue').innerHTML = averageRoundTripTime + 'ms';
